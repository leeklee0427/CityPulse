"""
File: average_household_income.py
Author: Louie SUN (hongyuns), Daochen LI (daochenl)
Description: Retrieve city and average household income data from Census using API
...
url_raw = ("https://api.census.gov/data/2022/acs/acs1/profile?get=NAME,"
           "DP03_0063E&for=place:*&in=state:*&key=f8579687edb9f7e019a2b54348ff323abd3a0a53")
...
def fetch_data(place_code, state_code):
    df = pd.read_csv("./data/raw/average_household_income_raw.csv")
    ...
    filtered_df.rename(columns={'DP03_0063E': 'Income'}, inplace=True) # Average household income
...
def fetch_income():
    ...
    if response_raw.status_code == 200:
        try:
            ...
            df_raw.to_csv("./data/raw/average_household_income_raw.csv")
    ...
    filename = './data/clean/average_household_income.csv'
